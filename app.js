...
function normalizeNode(n) {
  const node = { kind: 'box', family: 'information', text: '', value: true, min: 0, max: 100, formula: 'x + y', fileName: '', fileData: '', fileType: '', url: '', date: '', week: '', width: 220, height: 146, fontSize: 12, muted: false, comment: '', status: 'draft', activityTags: {}, ...n };
  node.activityTags = normalizeActivityTags(node.activityTags);
...
function setupToolPalette() {
  ...
  workspace.addEventListener('drop', e => {
    ...
    if (!['box', 'panel', 'slider', 'switch', 'add', 'subtract', 'multiply', 'divide', 'formula', 'pdf', 'link', 'milestone'].includes(tool)) return;
